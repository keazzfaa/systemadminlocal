...
export const ROLES = {
  ...
  admin: { label: "Administrator", color: "navy",  perms: ["dashboard","inventory","cashflow","reports","supplier","customer","audit","users"] },
  sales: { label: "Sales",         color: "green", perms: ["dashboard","inventory","sales","customer","barang-keluar","barang-masuk"] },
};

const SEED_USERS = [
  { id:1, name:"Super Owner",  email:"owner@neuralops.id",  role:"owner", avatar:"SO", password:"owner123"  },
  { id:2, name:"Marcus Tan",   email:"admin@neuralops.id",  role:"admin", avatar:"MT", password:"admin123"  },
  { id:3, name:"Sari Dewi",    email:"sales@neuralops.id",  role:"sales", avatar:"SD", password:"sales123"  },
];
...
export function AppProvider({ children }) {
  ...
  const [users, setUsers]       = useState(() => {
    const saved = localStorage.getItem("no_users");
  ...
  const saveUsers = (list) => {
    setUsers(list);
    localStorage.setItem("no_users", JSON.stringify(list));
  };
  ...
  const login = (email, password) => {
    ...
    if (!u) return false;
    setSession(u);
    localStorage.setItem("no_session", JSON.stringify(u));
  ...
  const toggleDark = () => setDarkMode(d => {
    const nv = !d;
    localStorage.setItem("no_dark", nv ? "1" : "0");
    return nv;
  });
  ...
  return (
    <AppContext.Provider value={{
      session, login, logout, register, darkMode, toggleDark, hasPerm, ROLES,
      users, setUsers: saveUsers,
