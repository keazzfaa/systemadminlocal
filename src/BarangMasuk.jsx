...
export default function BarangMasuk() {
  ...
  const filtered = itemsIn.filter(x =>
  ...
  const openAdd = () => { setForm({...EMPTY, tanggal:new Date().toISOString().slice(0,10), kode:"IN-"+String(itemsIn.length+1).padStart(3,"0")}); setEditId(null); setModal(true); };
  ...
  const save = () => {
    ...
    if (editId) {
      setItemsIn(prev => prev.map(x => x.id===editId ? item : x));
      addLog("Edit Barang Masuk", `${item.kode}: ${item.nama}`, "inventory");
    } else {
      setItemsIn(prev => [item, ...prev]);
      addLog("Tambah Barang Masuk", `${item.kode}: ${item.qty} ${item.satuan} ${item.nama}`, "inventory");
    }
    setModal(false);
  };

  const remove = (id) => {
    const item = itemsIn.find(x=>x.id===id);
    setItemsIn(prev=>prev.filter(x=>x.id!==id));
    addLog("Hapus Barang Masuk", `${item.kode}: ${item.nama}`, "inventory");
    setDel(null);
  };

  const doExport = () => {
    exportToExcel(filtered, "Barang_Masuk", ["id","tanggal","kode","nama","qty","satuan","harga","supplier","total","keterangan"]);
    addLog("Export Excel", "Barang Masuk diekspor", "export");
  };
  ...
  return (
    <>
      ...
      <div className="summary-row">
        ...
        <div className="card stat-card">
          <h3>Total Nilai</h3>
          <strong style={{fontSize:22}}>{FMT(totalNilai)}</strong>
        </div>
        <div className="card stat-card">
          <h3>Total Qty Masuk</h3>
          <strong>{filtered.reduce((s,x)=>s+x.qty,0)} unit</strong>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <h2>{editId?"Edit":"Tambah"} Barang Masuk</h2>
            ...
            <div className="form-grid2">
              {[
                ...
                ["Qty","qty","number"],["Satuan","satuan","text"],["Harga Satuan","harga","number"],
              ].map(([label,key,type])=>(
                <div key={key}>
                  <label>{label}</label>
              ...
              <div>
                ...
                <select className="field" value={form.supplier} onChange={e=>setForm(f=>({...f,supplier:e.target.value}))}>
